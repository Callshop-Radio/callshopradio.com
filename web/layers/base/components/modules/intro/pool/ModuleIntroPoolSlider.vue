...
const scrollTo = (index: number) => {
	...
	currentIndex.value = index; // Update currentIndex on manual scroll
};
...
// Use the pool items from the props
const slides = computed(() => {
	const result = [];
	// Group pool items into pairs (2 per slide)
	for (let i = 0; i < props.poolItems.length; i += 2) {
		const pair = [props.poolItems[i]];
		// Add the second pool item if available
