...
export default defineNitroPlugin(async (nitroApp) => {
  ...
  nitroApp.hooks.hook('close', async () => {
    try {
      console.log('🗺️ Generating static sitemap.xml for Callshop Radio...')
      ...
      try {
        ...
        console.warn('Warning: Could not fetch sitemap data from Sanity:', sanityError)
        sitemapData = []
      }
...
${uniqueRoutes.map(route => `  <url>
    ...
    <priority>${route.priority}</priority>
      ...
      console.log(`✅ Static sitemap.xml generated with ${uniqueRoutes.length} unique routes`)
      
      // Log content types for debugging
      const contentTypes = uniqueRoutes.reduce((acc, route) => {
        const type = route._type || 'static'
        acc[type] = (acc[type] || 0) + 1
        return acc
      }, {})
      console.log('📊 Content types in sitemap:', contentTypes)
  ...
  <url>
    ...
    <priority>0.8</priority>
      ...
      try {
        ...
        console.log('✅ Fallback sitemap.xml generated')
      } catch (writeError) {
